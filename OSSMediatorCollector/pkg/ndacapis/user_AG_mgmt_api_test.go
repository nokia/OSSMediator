...
var (
	fetchAccUUIDResp = `
	{
  		"status": {
    		...
    		"status_description": {
      			"description_code": "NOT_SPECIFIED",
      			"description": "Successfully fetched accounts mapped to user's organization"
    		}
  		},
  		"account_details": [
    		{
      			"account_uuid": "test_acc_uuid_1",
      			"account_alias": "ACC_UUID_1"
    		},
    		{
      			"account_uuid": "test_acc_uuid_2",
      			"account_alias": "ACC_UUID_2"
    		}
  		]
	}`
)
...
func TestFetchOrgUUID(t *testing.T) {
	...
	_, err := fetchOrgUUID(&config.APIConf{API: "/getOrgDetail", Interval: 15}, &user, 1234)
	if err != nil {
		t.Fail()
	}
}
...
func TestFetchOrgUUIDForInvalidCase(t *testing.T) {
	...
	fetchOrgUUID(&config.APIConf{API: "/getOrgDetail", Interval: 15}, &user, 1234)
...
func TestFetchAccUUID(t *testing.T) {
	...
	_, err := fetchAccUUID(&config.APIConf{API: "/getAccDetail", Interval: 15}, &user, orgDet, 1234)
	if err != nil {
		t.Fail()
	}
}
...
func TestFetchAccUUIDWithInactiveSession(t *testing.T) {
	...
	_, err := fetchAccUUID(&config.APIConf{API: "/getOrgDetail", Interval: 15}, &user, orgDet, 1234)
	if err != nil || !strings.Contains(buf.String(), "Skipping API call for testuser@nokia.com") {
		t.Fail()
	}
}
...
func TestFetchAccUUIDForInvalidCase(t *testing.T) {
	...
	CreateHTTPClient("", true)
	fetchAccUUID(&config.APIConf{API: "/getAccDetail", Interval: 15}, &user, orgDet, 1234)
	if len(user.NhgIDsABAC) != 0 {
		t.Fail()
	}
}
...
func TestFetchAccUUIDWithInvalidResponse(t *testing.T) {
	...
	CreateHTTPClient("", true)
	fetchAccUUID(&config.APIConf{API: "/getOrgDetail", Interval: 15}, &user, orgDet, 1234)
	if !strings.Contains(buf.String(), "Unable to decode response") {
		t.Fail()
	}
	if len(user.NhgIDsABAC) != 0 {
