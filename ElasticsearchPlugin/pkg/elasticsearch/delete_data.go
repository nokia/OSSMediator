...
func deleteIndices(indices []string, esConf config.ElasticsearchConf) {
	...
	//closing the indices
	_, err := httpCall(http.MethodPost, elkURL+"/"+"_close", esConf.User, esConf.Password, "", queryParams, defaultTimeout)
	if err != nil {
	...
	_, err = httpCall(http.MethodDelete, elkURL, esConf.User, esConf.Password, "", queryParams, defaultTimeout)
	if err != nil {
...
func getOldIndices(esConf config.ElasticsearchConf) []string {
	...
	//fetch the indices
	respBody, err := httpCall(http.MethodGet, elkURL, esConf.User, esConf.Password, "", nil, defaultTimeout)
	if err != nil {
	...
	for _, index := range indices {
		...
		indexCreationMonth, err := strconv.Atoi(splits[len(splits)-2])
		if err != nil {
			continue
		}
		indexCreationYear, err := strconv.Atoi(splits[len(splits)-1])
		if err != nil {
			continue
		}
