...
Check if execute permissions are there for the `storesecret` binary, if not set it as `chmod 777 storesecret`, then execute `sudo ./storesecret -conf_file=collector_conf.json` command to store the user passwords.  
...
* In case the user’s password is updated, execute `sudo ./storesecret -conf_file=collector_conf.json` and input the updated password, then execute `sudo ./startup.sh` to restart the modules again.
